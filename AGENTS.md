...
Rozšiřuje globální pravidla (`~/.claude/`, načítají se sama), **nepřevažuje je.**
Jen AI overlay (instrukce, code style, makra, key files). Fakta o projektu (identita,
stav, hierarchie modelu, milníky, changelog) patří do `README.md` / `docs/DIARY.md` /
`docs/DESIGN_DECISIONS.md`.

**`CLAUDE.md` je jen rozcestník, který tenhle soubor importuje** — needituj ho.
Dvě plné kopie overlaye se rozešly (zjištěno 2026-07-12, Key Files); od té doby
je zdroj jeden.
...
| sim | `sim/trackData.ts` | asymetrická Bernoulliho lemniskáta + C² spojka 2→1, dvě výhybky (`SWITCH_U`); výška `Y` z terénu + most u křížení (DD-20/DD-26); `buildLoopNetwork` (graf) |
| sim | `sim/terrain.ts` | `terrainHeight` — výšková matematika, single source pro sim (trať) i view (mesh), DD-20 |
| sim | `sim/serviceSite.ts` | servisní bod na branch trase (vodní jeřáb, S41): poloha + doplňování uhlí/vody/písku, sdílí sim i view |
...
`%AUDIT:CODE`, `%AUDIT:DOCS`, `%CALIBRATE:PROJ`) → globální `~/.claude/PROMPTS.md`,
včetně pravidla skládání projektových a globálních maker (sekce *Skládání
s projektovými makry*).
