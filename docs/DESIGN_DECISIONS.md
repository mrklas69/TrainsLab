...
| DD-22 | S22 | **Modely vozů = view factory (`carModels.ts`); typ vozu je view metadata 1:1 s tělesy.** Sim zná jen 1D těleso (délka+hmota); vzhled, valení kol, hnací spojnice i vizualizace prokluzu (`driverSlipPhase`) jsou ryze view (z `body.s` + flagu `slipping`). | Drží DD-01/DD-02 — vizuál a jeho pohyb nevstupují do simu. Izomorfní s DD-19 (dron): druhá vrstva čistě-view dat. `carTypes` jdou do Rendereru paralelně s `carLengths` (ty do simu). Render loop operuje na `group` místo `Mesh` (transformace beze změny). |
| DD-23 | S23 | **Rytmus parního výfuku jako sdílený view zdroj (`ExhaustClock`), fyzikálně odvozený.** Frekvence výfuku = otáčky kola × 4 (dvojčinná dvojválcová mašina), otáčky = `v/(π·D)`; `main` posouvá fázi jednou za frame, obě view čtou flag `fired`. Kouř (`SmokeView`, faceted obláčky ve world-space → vlečka emergentně) i zvukový chuff pufají z téhož taktu. | „Sladěný se zvukem" = jeden zdroj rytmu, ne dva časovače (DRY/conceptual integrity) — jinak by se fáze rozjely. Fyzikální odvození > fenomenologie (`0.9/(v+0.4)`): emergentně dá pomalý rozjezdový „čch… čch…" i hustý sykot, bez ladění konstanty. Drží DD-01 (rytmus je view, ne sim). World-space kouř = vlečka bez skriptu (emergence = nika projektu). |
| DD-21 | S19 | **Rázy z trati jako impulsy do existujících kývacích oscilátorů** — dilatační spáry (→ pitch, perioda `railLength`), skok křivosti / výhybky (→ roll ve směru oblouku, `TRACK_PERTURBATIONS`); jeden `crossed()` test (floor-trik na arc-length). κ-skok **fenomenologicky** (roll-kick na nábězích laloků), ne přepis geometrie lemniskáty. | Rozšiřuje DD-13 bez nového DOF (recyklace roll/pitch) → drží DD-02 (mění jen rotaci). Tři jevy = jeden mechanismus (KISS/DRY); per-vůz `s` → ráz proběhne soupravou jako vlna (emergence). Fenomenologie drží hladkou lemniskátu i roh mřížky (okolky = Úr. 3, jinam). |
