...
const SECTIONS = [
    ...
    },
    {
        ...
        sliders: [
            ...
            { key: 'couplerStiffness', label: 'Tuhost', min: 0, max: 5000000, step: 100000, unit: 'N/m' },
            { key: 'couplerDamping', label: 'Tlumení', min: 0, max: 300000, step: 5000, unit: 'N·s/m' },
        ],
    },
    {
        title: 'Trakce & adheze',
        sliders: [
            { key: 'maxPower', label: 'Výkon', min: 0, max: 2000000, step: 50000, unit: 'W' },
            { key: 'tractiveForceMax', label: 'Max tažná síla', min: 0, max: 400000, step: 10000, unit: 'N' },
            { key: 'adhesionCoeff', label: 'Adheze μ', min: 0, max: 0.5, step: 0.01, unit: '' },
            { key: 'brakeForceMax', label: 'Brzda', min: 0, max: 400000, step: 10000, unit: 'N' },
        ],
    },
];
/**
 ...
 */
export function createControlPanel(params, controls) {
    ...
    for (const section of SECTIONS) {
        ...
        for (const def of section.sliders)
            panel.appendChild(buildSlider(params, def));
    }
    ...
    return (train) => {
        ...
        const flags = (train.isBraking ? ' · BRZDA' : '') + (train.slipping ? ' · PROKLUZ' : '');
...
function buildSlider(params, def) {
